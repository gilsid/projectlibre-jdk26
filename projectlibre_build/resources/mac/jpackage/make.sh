...
"${JPACKAGE}" --type app-image --input source --dest app --name ProjectLibre \
...
create-dmg --volname "ProjectLibre" --background "background.png" \
  --volicon "source/projectlibre.icns" --window-pos 200 120 \
  --window-size 600 350 --icon-size 100 --icon "ProjectLibre.app" 180 140 \
  --hide-extension "ProjectLibre.app" --app-drop-link 420 140 \
  --eula source/license/license.txt "ProjectLibre-${APP_VERSION}.dmg" "app"
codesign --force --timestamp --options runtime \
  --sign "Developer ID Application: ProjectLibre Inc. ($APPLE_TEAM_ID)" \
  "ProjectLibre-${APP_VERSION}.dmg"
xcrun notarytool submit "ProjectLibre-${APP_VERSION}.dmg" \

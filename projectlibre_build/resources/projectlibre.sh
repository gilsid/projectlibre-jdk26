...
if [ -f "$RUN_CONF" ]; then
	. "$RUN_CONF"
	PROJECTLIBRE_HOME="$PROJECTLIBRE_HOME0"
	# Revalidate the cached runtime. A JDK upgrade must not reuse an old JVM.
	if [ -x "$JAVA_EXE" ] || command -v "$JAVA_EXE" >/dev/null 2>&1; then
		check_java "$JAVA_EXE"
	else
		JAVA_OK="0"
	fi
	if [ "$JAVA_OK" -eq "1" ]; then
		run_projectlibre --silentlyFail true "$@"
		exit $?
	fi
fi
...
if [ "$JAVA_OK" -eq "0" ]; then
	...
	if [ "$JAVA_OK" -eq "0" ]; then
		...
		echo "Please install OpenJDK 26 or set JAVA_HOME to a Java 26 installation."
	else
		echo "Java OK"
	fi
